# torchcanvas_all_in_one.py
# ------------------------------------------------------------
# TorchCanvas: Graph → PyTorch nn.Module → Export(TorchScript/ONNX)
# 요구: torch >= 1.10 (LazyConv/LazyLinear/LazyBatchNorm 사용)
# 실행: python torchcanvas_all_in_one.py
# 결과: ./torchcanvas_demo.pt / ./torchcanvas_demo.onnx 생성
# ------------------------------------------------------------
...
def _conv1d(out_channels:int, kernel_size:int, stride:int=1, padding:str|int="same", **kwargs):
    pad = kernel_size // 2 if padding == "same" else int(padding)
...
class InferenceWrapper(nn.Module):
    """GraphModule(dict in/out)를 단일 텐서 입출력으로 감싸는 래퍼"""
    def __init__(self, gmod: GraphModule, input_key="inp", output_key="fc"):
        super().__init__()
        self.gmod = gmod
...
def to_torchscript(gmod: GraphModule, example: torch.Tensor, out_path="torchcanvas_demo.pt"):
    gmod.eval()
    with torch.no_grad():
...
def to_onnx(gmod: GraphModule, example: torch.Tensor, out_path="torchcanvas_demo.onnx", opset=17):
    gmod.eval()
    with torch.no_grad():
        _ = gmod({"inp": example})  # Lazy materialize
        wrapped = InferenceWrapper(gmod)
...
# ------------------------------
# 5) 데모 실행 (그래프 구성 → 실행 → Export)
# ------------------------------
if __name__ == "__main__":
    # 예제 그래프: (B,C,T) → Conv1d → ReLU → Permute(B,T,C) → GRUBlock(last) → Linear → 출력
    g = GraphDef(
        nodes=[
            Node("inp", "Input"),
    ...
    out = gmod({"inp": x})["fc"]
    print("out.shape =", out.shape)  # 기대: torch.Size([4, 18])

    # Export (TorchScript / ONNX)
    ex = torch.randn(1, 9, 256)  # 예시 입력 (동적 배치/시간축 지정)
    to_torchscript(gmod, ex, "torchcanvas_demo.pt")
    to_onnx(gmod, ex, "torchcanvas_demo.onnx")
